...
def fetch_prices_for_batch(
    ...
    """
    Returns list of rows matching (ticker_id, date, open, high, low, close, adj_close, volume)
    """
    ...
    try:
        ...
        for t, tdf in per_ticker.items():
            ...
            # Compute adjusted close using dividends and splits
            # Ensure index is sorted ascending by date
            tdf = tdf.sort_index()

            div = (
                tdf["Dividends"]
                if "Dividends" in tdf.columns
                else pd.Series(0.0, index=tdf.index)
            )
            splits_col = (
                tdf["Stock Splits"]
                if "Stock Splits" in tdf.columns
                else pd.Series(0.0, index=tdf.index)
            )

            close_series = tdf["Close"].copy()

            # Build cumulative adjustment factor from most recent to oldest
            cumulative = 1.0
            adj_factors = []  # same order as reversed index
            for idx in reversed(close_series.index):
                c_val = close_series.loc[idx]
                d_val = 0.0
                s_val = 0.0
                try:
                    d_val = float(div.loc[idx]) if pd.notna(div.loc[idx]) else 0.0
                except Exception:
                    d_val = 0.0
                try:
                    s_val = (
                        float(splits_col.loc[idx])
                        if pd.notna(splits_col.loc[idx])
                        else 0.0
                    )
                except Exception:
                    s_val = 0.0

                # Set adjusted close for this date (using cumulative factor for prior future events)
                try:
                    c_float = float(c_val) if pd.notna(c_val) else None
                except Exception:
                    c_float = None
                if c_float is None:
                    adj_factors.append((idx, None))
                else:
                    adj_factors.append((idx, c_float * cumulative))

                # Update cumulative with today's events for earlier dates
                # Split factor (apply to earlier dates): multiply by 1/ratio if ratio != 0 and != 1
                if s_val and s_val != 0.0 and s_val != 1.0:
                    try:
                        cumulative *= 1.0 / s_val
                    except Exception:
                        pass

                # Dividend factor: factor = Close / (Close + Dividend)
                if c_float is not None and d_val and d_val != 0.0:
                    denom = c_float + d_val
                    if denom > 0:
                        cumulative *= c_float / denom

            # Build a Series of computed adjusted close values in index order
            adj_map = {idx: val for idx, val in adj_factors}
